...
page_init(void) {
    ...
    for (i = 1; i < npages_basemem; i++) {
        // 0x7000
        // 系统通过BSP引导之后，剩下的被激活的AP处理器就不用引导系统了，而是直接把地址为0x7000作为入口地址，开始执行系统。
        // 防止这个地址被改为空页
    ...
    // 第一页前表示已经用过了
    // boot_alloc返回的是内核虚拟地址，PADDR就是转成物理地址，实际上就是减去KERNBASE
    size_t first_page = PGNUM(PADDR(boot_alloc(0)));
    for (; i < first_page; i++) {
        pages[i].pp_ref = 1;
    }
    // 其他页为空
...
mmio_map_region(physaddr_t pa, size_t size) {
    ...
    if (base >= MMIOLIM)
        panic("Overflow in MMIOLIM");

    boot_map_region(kern_pgdir, res, size, pa, PTE_PCD | PTE_PWT | PTE_W);
